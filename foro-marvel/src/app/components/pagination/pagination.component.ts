...
export class PaginationComponent implements OnInit, OnChanges {
  ...
  calcularPaginas() {
    ...
    this.registroHasta =
      this.registroActual + this.registrosPorPagina - 1 <= this.totalRegistros
    ...
    this.paginaActual =
      Math.trunc(this.registroActual / this.registrosPorPagina) + 1;
    // Calculamos el total de páginas
    this.ultimaPagina =
      Math.trunc((this.totalRegistros - 1) / this.registrosPorPagina) + 1;
    // Calculamos el desde - hasta de las páginas a generar
    const desde =
      this.paginaActual - this.prepost > 0
        ? this.paginaActual - this.prepost
        : 1;
    const hasta =
      this.paginaActual + this.prepost < this.ultimaPagina
        ? this.paginaActual + this.prepost
        : this.ultimaPagina;
